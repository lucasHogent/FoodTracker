...
class ProductRepository @Inject constructor(
        ...
        return withContext(Dispatchers.IO) {
            ...
            if (isInternetConnected()) {
                try {
                    ...
                    Timber.i(
                        "Get products from API getProducts() %s",
                    ...
                    Timber.i(
                        "Insert products from API to productDao.insertAll %s",
                        filteredProductEntities.toTypedArray().contentToString()
                    )
                } catch (e: IOException) {
                    Timber.e(e.message)
    ...
    override suspend fun getCountDBProducts(): Int {
        var count = productDao.getAllProducts().count()
        return count
    }

    override suspend fun updateProduct(product: ProductDetailModel) {
        val productEntity = product.asEntity()
        Timber.i("Update product %s", product)
    ...
    override suspend fun clear() {
        Timber.i("Clear DB for Products")
        productDao.clear()
    }
        ...
        return withContext(Dispatchers.IO) {
            ...
            if (isInternetConnected()) {
                try {
                    ...
                    Timber.i(
                        "Get products from API getProducts(title) %s",
                        productsFromApi.results.toTypedArray()
                    )
